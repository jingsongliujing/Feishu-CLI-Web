...
const larkSetupTitle = computed(() => (larkSetupScopes.value.length ? '补充飞书权限' : '连接飞书账号'))
...
const applyLarkSetupMetadata = (metadata?: Record<string, any>) => {
  ...
  if (Array.isArray(metadata.setup_steps)) {
    larkSetupSteps.value = metadata.setup_steps.map((step: any) => ({
      ...step,
...
const startLarkSetup = async () => {
  ...
  try {
    const response = await fetch('/api/v1/lark/setup/stream', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...authHeaders() },
      body: JSON.stringify({ user_id: currentUserId.value, scopes: larkSetupScopes.value })
    })
    if (!response.ok || !response.body) throw new Error(`HTTP error! status: ${response.status}`)
    const reader = response.body.getReader()
    const decoder = new TextDecoder()
    ...
    while (true) {
      ...
      for (const block of blocks) {
        ...
        const data = JSON.parse(line.slice(5).trim())
        if (data.type === 'status') {
          larkSetupSteps.value = (data.steps || []).map((step: any) => ({
            key: step.key,
            title: step.title,
            command: step.display_command || step.command || '',
            status: 'pending'
          ...
          if (data.success) showLarkSetup.value = false
        }
      }
    }
  } catch (error: any) {
...
onMounted(async () => {
  ...
  await Promise.all([loadHistory(), refreshLarkSetup(), loadModelConfig()])
          ...
          <div v-if="showLarkSetup" class="clarify-container">
            <div class="clarify-card lark-setup-card">
              ...
              <div v-if="larkSetupSteps.length" class="lark-setup-steps">
                <div v-for="step in larkSetupSteps" :key="step.key" class="lark-setup-step" :class="step.status">
                  <div class="lark-setup-step-title">{{ step.title }}</div>
                  <div class="lark-setup-step-command">{{ step.command }}</div>
                </div>
              </div>
              <div class="write-confirm-actions">
                <button type="button" class="write-confirm-btn" :disabled="larkSetupRunning" @click="startLarkSetup">
                  {{ larkSetupRunning ? '处理中...' : '开始连接' }}
                </button>
              </div>
